...
from scripts.analyze_trades import (
    ...
    estimate_trade_cost_from_event,
...
from scripts.models import TradeEventRow
...
class AnalyzeTradesTests(unittest.TestCase):
    ...
    def test_build_performance_summary_row_calculates_core_metrics(self):
        trades = [
            {
                "trade_id": "t1",
                "entry_time": "2026-03-28 10:00:05",
                "exit_time": "2026-03-28 10:05:05",
                "time_in_trade_min": 5,
                "trade_type": "SELL_PE",
                "strike": "22500",
                "expiry": "2026-03-30",
            ...
            {
                "trade_id": "t2",
                "entry_time": "2026-03-28 11:00:05",
                "exit_time": "2026-03-28 11:04:05",
                "time_in_trade_min": 4,
                "trade_type": "SELL_CE",
                "strike": "22450",
                "expiry": "2026-03-30",
            ...
            {
                "trade_id": "t3",
                "entry_time": "2026-03-29 10:00:05",
                "exit_time": "2026-03-29 10:10:05",
                "time_in_trade_min": 10,
                "trade_type": "SELL_PE",
                "strike": "22600",
                "expiry": "2026-03-30",
        ...
        row = build_performance_summary_row("sell", trades)

        self.assertEqual(row["side"], "sell")
        self.assertEqual(row["trading_days"], 2)
        self.assertEqual(row["total_trades"], 3)
        self.assertEqual(row["winning_trades"], 1)
        self.assertEqual(row["losing_trades"], 1)
        self.assertEqual(row["flat_trades"], 1)
        self.assertEqual(row["gross_pnl"], 25.0)
        self.assertEqual(row["gross_profit"], 150.0)
        self.assertEqual(row["gross_loss"], 125.0)
        self.assertEqual(row["estimated_cost"], 300)
        self.assertEqual(row["net_after_cost"], -275.0)
        self.assertEqual(row["profit_factor"], 1.2)
        self.assertEqual(row["best_trade"], 150.0)
        self.assertEqual(row["worst_trade"], -125.0)
        self.assertEqual(row["best_day"], 25.0)
        self.assertEqual(row["worst_day"], 0.0)
        self.assertEqual(row["max_drawdown"], -125.0)
        self.assertEqual(row["max_loss_streak"], 1)

    def test_estimate_trade_cost_from_event_uses_dhan_style_formula(self):
        row = TradeEventRow(
            side="SELL",
            trade_id="t1",
            regime="SELL_PE",
            entry_signal="SELL_PE",
            entry_time="2026-04-02 10:00:00",
            exit_time="2026-04-02 10:05:00",
            time_in_trade_min=5,
            strike="22500",
            expiry="2026-04-30",
            entry_spot=22500.0,
            entry_direction_score=0.5,
            entry_bias=20.0,
            entry_price=700.0,
            exit_reason="TEST",
            trade_pnl=250.0,
        ...
        self.assertEqual(estimate_trade_cost_from_event(row), 112.11)
