...
class BasePaperTradeEngine:
    ...
    def exit_position(self, now: datetime, reason: str, ltp_map=None):
        ...
        entry_price = 0.0
        exit_price = 0.0
        quantity = CONFIG.lot_size * p.lots
        pos = safe_load_json(self.get_open_position_file(), {}) or {}
        legs = pos.get("legs", [])
        if legs:
            active_leg = legs[0]
            entry_price = float(active_leg.get("entry_price") or 0.0)
            active_sid = int(active_leg.get("security_id") or 0)
            exit_price = float(ltp_map.get(active_sid) or 0.0)
